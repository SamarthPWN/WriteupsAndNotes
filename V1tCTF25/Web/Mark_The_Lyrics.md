...
**Website:** http://tommytheduck.github.io/mckey

## Writeup:
Upon opening the provided challenge link, the page displayed an embedded YouTube video titled `2323 - NGHIÊM TỔNG prod. MAI CẢNH DỊ & ĐẠI TRƯỢNG FU` and a section header labeled `2323 - MCK`. Below that were what seemed to be lyrics of a Vietnamese song, including various artist credits like `Sơn Tùng M-TP` and `RPT MCK`. At first glance, nothing appeared out of the ordinary, just neatly formatted lyrics with sections such as Intro, Verse, Pre-Chorus, Chorus and so on. Since the challenge title was `Mark The Lyrics`, I suspected that something might be hidden within the way the lyrics were presented, perhaps using HTML `<mark>` tags or annotations.  
To explore this idea, I checked the site’s structure. The domain `tommytheduck.github.io` hinted that this was a GitHub Pages site, meaning the HTML source should exist in one of the public repositories of the user `tommytheduck`. I went directly to `https://github.com/tommytheduck` to inspect their repositories. Among them, I found one named `mckey`, which clearly matched the challenge link (`/mckey` corresponds to the repository name used for the GitHub Pages path). Opening this repository, I located an `index.html` file, the main page source that powers the hosted site.  
Inside the `index.html` file, I found the following structure:
```html
<!DOCTYPE html>
<html lang="vi">
<head>
...
```
After scrolling through the HTML, I noticed several `<mark>` tags scattered across the document. These tags are typically used to highlight text, but here they had a white background color, making the highlighted parts blend seamlessly with the rest of the text. That immediately stood out because the challenge name `Mark The Lyrics` was a clear hint to look at these marks. Each `<mark>` contained one or more characters that seemed out of place or emphasized.  
Listing all the marked text in order of appearance gave me the following sequence:  
`V`, `1`, `T`, `{`, `MCK`, `-pap-`, `cool`, `-ooh-`, `yeah`, `}`  
When arranged together, it formed the flag pattern clearly resembling the expected format.
